...
DECLARE
  ...
  -- plausible ceiling for yield per hectare (kg) used to clamp outliers
  c_yield_per_ha_cap constant numeric := 12000;
  c_yield_benchmark  constant numeric := 2000;
  c_income_band      constant numeric := 5000; -- USD/yr for full income credit
BEGIN
  ...
  -- Treat non-positive farm size as missing
  ...
  -- Yields: ignore impossible years, clamp implausible volumes
  ...
  -- Farming methods: only recognised methods count
  ...
  -- Financial health: banded income + verified banking evidence
  ...
  END IF;
  IF v_revenue > 0 THEN
    v_financial := v_financial + 15; -- corroborating recorded crop revenue
  END IF;
  ...
  IF NOT v_has_fin_doc THEN
    -- self-reported only: cap until financial evidence is verified
  ...
  -- Confidence: share of key fields present, weighted toward verified evidence
  -- self-reported fields = 1pt, verified evidence = 2pt
  v_conf_max := 16;
  ...
  -- Deterministic recommendations, weakest first
  IF v_yield_history < 80 THEN
    v_rec_list := v_rec_list || 'Record more crop yield history (4+ years strengthens the score)';
  END IF;
  IF v_years < 2 THEN
    v_rec_list := v_rec_list || 'Capture at least two seasons of yields to assess growth';
  END IF;
  IF v_size <= 0 THEN
    v_rec_list := v_rec_list || 'Capture an accurate farm size in hectares';
  END IF;
  IF v_methods = 0 THEN
    v_rec_list := v_rec_list || 'Record a recognised farming method per crop';
  END IF;
  IF NOT v_has_fin_doc THEN
    v_rec_list := v_rec_list || 'Upload and verify a bank or financial statement to unlock full financial scoring';
  END IF;
  IF v_verified_docs = 0 THEN
    v_rec_list := v_rec_list || 'Upload identification and land documents for verification';
  END IF;
  IF v_confidence < 50 THEN
    v_rec_list := v_rec_list || 'Data confidence is below 50% — complete the farmer record before relying on this score';
  ELSE
  ...
  VALUES
    (_farmer_id, v_farmer.organization_id, v_score, v_band, v_breakdown, v_recs, v_inputs_hash, auth.uid(), now(), 'v2-server')
