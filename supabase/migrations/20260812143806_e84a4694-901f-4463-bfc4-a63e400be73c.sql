...
BEGIN
  ...
  IF NOT public.has_permission(v_caller, 'team.manage_roles', _org_id) THEN
...
BEGIN
  ...
  IF NOT public.has_permission(v_caller, 'team.revoke', v_inv.organization_id) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
...
RETURNS TABLE(user_id uuid, full_name text, email text, created_at timestamp with time zone, last_sign_in_at timestamp with time zone, roles app_role[])
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path = public
AS $function$
BEGIN
  IF NOT public.has_permission(auth.uid(), 'team.view', _org_id) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
