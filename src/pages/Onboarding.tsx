...
export default function Onboarding() {
  ...
  const validateStep = (step: number): { ok: boolean; missing: string[]; errs: Record<string, boolean> } => {
    ...
    if (step === 0) {
      ...
      const selected = [d.primaryCrop, d.secondaryCrop, d.thirdCrop].filter(Boolean);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const selectedCrops = [cr.primaryCrop, cr.secondaryCrop, cr.thirdCrop].filter(Boolean);
            ...
            return (
              <div key={step.id} className="flex items-center flex-1">
                <button
                  ...
                  className={cn(
                    "flex items-center justify-center sm:justify-start gap-2 rounded-lg px-2 sm:px-3 py-2 text-xs font-medium transition-colors w-full",
                    active && "bg-primary text-primary-foreground",
                    done && "bg-kyf-sage-light text-primary",
                    !active && !done && "bg-muted text-muted-foreground"
                  )}
                >
                  {done ? <Check className="h-3.5 w-3.5" /> : <step.icon className="h-3.5 w-3.5 shrink-0" />}
